...
public class RedisConnectionPoolConfig {
	...
	public static JedisPoolConfig getJedisPoolConfigInstance(){
		if(_instance == null){
			try {
				String json = FileUtils.readFileAsString(REDIS_CONNECTION_POOL_CONFIG_FILE);
				_instance = new Gson().fromJson(json, RedisConnectionPoolConfig.class);
			}
			catch (Exception e) {
				if (e instanceof JsonSyntaxException) {
					e.printStackTrace();
					System.err.println("Wrong JSON syntax in file "+REDIS_CONNECTION_POOL_CONFIG_FILE);
				}
				else {
					e.printStackTrace();
				}
